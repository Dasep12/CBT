    ...
    $.ajax({
      ...
      success : function(response){
        ...
        for(var i = 0 ; i < response.length ; i++){
          data.push([ j , response[i].kode_tugas , response[i].judul_tugas , response[i].mata_pelajaran , response[i].kelas +" / "+ response[i].prodi , response[i].tanggal +"/"+ response[i].jam   , "<a href='javascript:;' data-id="+ response[i].id + "  data-toggle='modal' data-target='#lihat_tugas' class='btn btn-info btn-xs'>view</a><a href='javascript:del("+response[i].id +")' class='btn btn-danger btn-xs'>delete</a>" ])
        j++ ;
        }
           $('#tugas').DataTable({
            data : data 
